...
**Email/password only.** Google Sign-In requires custom native code, so it cannot run in Expo Go — adding it means a development build, a config plugin, SHA-1 fingerprints and Google Cloud configuration. That was judged out of scope against a term deadline, since the whole project is demonstrated through Expo Go.
...
- **Google Sign-In** — see the note above; it needs a development build rather than Expo Go.
